...
async def get_flightradar24_data():
    ...
    if cache_key in flightradar24_cache:
        logger.info("Devolviendo datos en caché de Flightradar24")
    ...
    try:
        ...
        for flight in flights:
            flight_info = {
                "flight_number": flight.callsign.strip() if flight.callsign else "N/A",
                ...
                "status": "En vuelo" if flight.on_ground == 0 else "En tierra",
                "source": "Flightradar24"
            }
            flight_data.append(flight_info)
        flightradar24_cache[cache_key] = flight_data
        logger.info(f"Datos de Flightradar24: {len(flight_data)} vuelos")
        return flight_data
    except Exception as e:
        logger.error(f"Error al obtener datos de Flightradar24: {str(e)}")
        return []
